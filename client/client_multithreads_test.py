...
def send_data(assets_dic):
    try:
        ...
        if RSA_status == 'RSA_OK': 
            ...
            sendSignalAndSize = 'AssetsDataCollect'+'|'+str(len(assets_dic_json))
...
def multi_client(m_dic, m_interval):
    #run single thread...
    def run(m_dic, m_interval):
        print 'going to run ...'
        monitor_api(m_dic, m_interval)
    result = [] 
    for i in range(100):
        result.append(threading.Thread(target=run,args=(m_dic,m_interval)).start())
    ...
    if len(interval_dic):
        for interval,monitor_dic in interval_dic.items():
            ...
            if time_diff >= interval:
                ...
                multi_client(monitor_dic, interval)
