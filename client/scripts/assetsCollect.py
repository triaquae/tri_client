...
def handle_info():
    ...
    monitor_info_dic['asset']={"SystemInformation":{},
                              # "LogicalDisks":value_dic['asset']['DxDiag'].get("LogicalDisks"),
                               "SystemDevices":value_dic['asset'].get('DxDiag')["SystemDevices"]}      
    
    
    for k in monitor_info_dic['asset'].keys():
	if k =="SystemInformation":
	    monitor_info_dic['asset'][k]={'MachineName':value_dic['asset'].get('DxDiag')["SystemInformation"]['MachineName'],
	...
	'''
	    ...
	    monitor_info_dic['asset'][k]={value_dic['asset'].get('DxDiag')["SystemDevices"]}
	                                         
	elif monitor_info_dic.get('asset')[k] =="LogicalDisks":
	    pass
	'''	 
    #get os info such:sn
    '''
    ...
    monitor_info_dic['asset']['OSInfo']=os_list
    ...
    monitor_info_dic['asset']['CPUInfo']=cpu_dic
    ...
    monitor_info_dic['asset']['LogicalDiskInfo']=logical_disk_list
    ...
    monitor_info_dic['asset']['PhysicalDiskInfo']=physical_disk_list
    ...
    monitor_info_dic['asset']['MemInfo']={"MemTotal":mem_dic["MemTotal"],
    ...
    monitor_info_dic['asset']['MemInfo']=mem_list
    ...
    monitor_info_dic['asset']['NetInfo']=tmplist
...
if __name__ == '__main__':
    ...
    for k in monitor_info_dic['asset']['SystemInformation'].keys():
	print "\t" + k + ":" + monitor_info_dic['asset']['SystemInformation'][k]        
    for value in monitor_info_dic['asset']['OSInfo']:
    ...
    '''
    print '\n系统设备信息:'.decode('utf-8')
    for k in monitor_info_dic['asset']['SystemDevices'].keys():
	for value in monitor_info_dic['asset']['SystemDevices'][k]:
	    print "Name:"
	    print value['Name']
    
    #print LogicalDisks
    print '系统逻辑磁盘信息\n'.decode('utf-8')
    for k in monitor_info_dic['asset']['LogicalDisks'].keys():
	for value in monitor_info_dic['asset']['LogicalDisks'][k]:
	    print value
    '''  
    #print hard disk info
    print '\n逻辑磁盘信息:'.decode('utf-8')
    for value in monitor_info_dic['asset']['LogicalDiskInfo']:
	if type(value) is dict:
	    for k in value.keys():
		print k+":" 
		print value[k]
    ...
    print '\n物理磁盘信息:'.decode('utf-8')
    for value in monitor_info_dic['asset']['PhysicalDiskInfo']:
	if type(value) is dict:
	    for k in value.keys():
		print k+":" 
		print value[k]	
    ...
    for k in monitor_info_dic['asset']['CPUInfo'].keys():
	print k + ":" 
	print monitor_info_dic['asset']['CPUInfo'][k]    
        
    #print meminfo
    print '\n内存信息:'.decode('utf-8')
    for value in monitor_info_dic['asset']['MemInfo']:
	if type(value) is dict:
	    for k in value.keys():
		print k+":" 
		print value[k] 
    ...
    #print NetInfo
    print '\n网卡信息:'.decode('utf-8')
    for value in monitor_info_dic['asset']['NetInfo']:
	if type(value) is dict:
	    for k in value.keys():
		print k + ":"
		print value[k]
	#print '\t'+ k + ":" 
	#print monitor_info_dic['asset']['NetInfo'][k]  
